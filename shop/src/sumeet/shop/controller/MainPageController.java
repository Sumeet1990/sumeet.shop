...
public class MainPageController {
		...
		for(ItemDetails item : itemsLst)
		{
			...
			if(name.startsWith(itemName) || desc.startsWith(itemDesc))
			{
				table = table	+"<tr>"
								...
								+"	<td align=\"right\"><input type=\"button\" onclick=\"updateRecord("+item.getItem_id()+")\" id=\"buttonAddDel\" value=\"View\"><input type=\"button\" onclick=\"deleteItem("+item.getItem_id()+")\" id=\"buttonAddDel\" value=\"Delete\"></td>"
								+"</tr>"
								+"<tr>";
	...
	public String searchCustomerAccount(@RequestParam("Customername") String Customername,
		...
		for(CustomerAccounts customer : customerAcc)
		{
				...
				table = table	+"<tr>"
						...
						+"	<td align=\"right\"><input type=\"button\" onclick=\"viewCustomerAcc("+customer.getCust_id()+")\" id=\"buttonAddDel\" value=\"View\"><input type=\"button\" onclick=\"deleteCustomer("+customer.getCust_id()+")\" id=\"buttonAddDel\" value=\"Delete\"></td>"
						+"</tr>";
	...
	@ResponseBody
	public String deleteCustomer(@RequestParam("cust_id") Integer cust_id, ModelMap map) {
		
		String status = DatabaseController.deleteCustomer(cust_id);
		return status;
	}
	
	@RequestMapping(value = "/deleteItem", method = RequestMethod.GET)
	@ResponseBody
	public String deleteItem(@RequestParam("itemId") Integer itemId, ModelMap map) {
		
		String status = DatabaseController.deleteItem(itemId);
		return status;
	}
